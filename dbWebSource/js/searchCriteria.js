...
function downloadGraphSummary(btn){
    ...
    var _paramValue = $(btn).closest(".option_box").find(".param_box").find("select").val();
    ...
    $.get(execURL + url, function(data){
        ...
        initGlobalData(function(){
            var _setSubCategory = function(el){
                var _d = [];
                var _$this = el;
                var _value = $.trim(_$this.val());
                var _$opBox = _$this.closest(".option_box");
                var _$subCategory = _$opBox.find("#sub_category");
                var _$subCategoryWrap = _$opBox.find("#wrap_sub_category");
                var _selected = _$subCategory.val();
                
                if( _value !== "Model Year" ){
                    if( _value === "Region" ){
                        _d = gRegionNames;
                    }else if( _value === "Market" ){
                        _d = gMarket;
                    }else if( _value === "Vehicle Type" ){
                        _d = gVehicleTypes;
                    }else if( _value === "OEM" ){
                        _d = gOEMs;
                    }
            ...
            _setSubCategory($(".collapse.show").find("select.category"));
            $("select.category").unbind().change(function(){
                _setSubCategory($(this));
            });
...
function getCriteriaParams(cid, callback){
    $.get(execURL + "dynamic_param_sel @criteria_id=" + cid
    , function(data){
        gCriteriaParams = data.rows;
        
        //GET CHART LIST
        $.get(procURL + "criteria_graphs_sel @criteria_id=" + cid
        , function(d){
            if(d.rows.length > 0){
                ...
                $(".section-card[cid*='"+ cid +"']").find("#graph_type").fillSelect({
                    data : _charts,
                    text : "graph_type",
                    value : "graph_type",
...
function getMathFunc(cid, callback){
    $.get(execURL + "dynamic_math_sel @criteria_id=" + cid
    , function(data){
        gMathFunc = data.rows;
...
function setCriteriaParams(callback){
    ...
    if(gCriteriaParams.length > 0){
        var _$optionBox = $(".section-card[cid='"+ gCId +"']").find(".option_box");
        var _$paramBox = _$optionBox.find(".param_box");
        $.each(gCriteriaParams, function(i, v){
            ...
            if(!_$paramBox.html()){
                var _tw = new zsi.easyJsTemplateWriter("#" + _$paramBox.attr("id"));
            ...
            if(!_selectedValue) _selectedValue = $.trim(_$paramBox.find("#" + _columnName).val());
            ...
            _$paramBox.find("#" + _columnName).fillSelect({
                data : _groupData,
                text : "name",
                value : "name",
...
function displayCharts(menuId, menuName, criteriaId, criteriaName, callback){
    getMathFunc(criteriaId, function(){
        getCriteriaParams(criteriaId, function(){
            getLegendData(criteriaId, function(){
...
function getChartSettings(o){
    if(!$.isEmptyObject(o)){
        ...
        if(isContain(_menuName, "WIRES & CABLES")){
            if(isContain(_cName, "New Conductor Technology with lesser dimensions")){
        ...
        _result.url = "dynamic_summary_sel @criteria_id="+ _cId +",@model_year_fr="+ gMYFrom +",@model_year_to="+ gMYTo + _param;
...
function resetDisplayChart(mid, cid, callback){
    var _ids = mid +"_"+ cid;
    var _$collapse = $("#collapse_"+ _ids);
   
    var _$chartDtlWrapper = _$collapse.find(".chart_details_wrapper");
    var _$btnReset = _$collapse.find(".btnReset");
    var _$chartDiv = _$collapse.find(".chart-div");
    var _$legendDiv = _$collapse.find(".legend-div");
    var _$trendId = _$collapse.find(".trend-div");
    var _$oppId = _$collapse.find(".opp-div");
    var _$sunburstDtlDiv = _$collapse.find(".sunburst_details");
    var _$barDtlDiv = _$collapse.find(".bar_details");
    var _$selectDtl = _$collapse.find(".chart_details_opt");

    _$btnReset.addClass("d-none");
    _$chartDtlWrapper.addClass("d-none");
    _$chartDiv.empty().width("auto");
    _$legendDiv.empty().width("auto");
    _$trendId.empty().width("auto");
    _$oppId.empty().width("auto");
    _$sunburstDtlDiv.empty().addClass("d-none").removeClass("d-flex");
    _$barDtlDiv.empty().addClass("d-none").removeClass("d-flex");
    _$selectDtl.val("sunburst");
    
    if(callback) callback();
}

//******************************* CHART FUNCTION *****************************//
function removeDuplicateLegend(chart){
...
function setLegend(charts, container){
    var _legend = [];
    var _chart = [];
    ...
    var _$chart = $("#"+ container);
    var _$chartWrap = _$chart.parent();
    var _$legend = $("#" + _legendDiv);
    var _$legendWrap = $(".legend-wrapper");
    
    _legendContainer();

    var _resizeLegend = function(ev) {
        if(_$chart.width() >_$chartWrap.width()){
            _$legendWrap.css("bottom", 16);
        }else{
            _$legendWrap.css("bottom", 0);
            _$chart.width(_$chart.width() - 1);
        }
    ...
    if(charts.length === 3){
        _$chartWrap.addClass("overflow-hidden");
    }else{
        _$chartWrap.removeClass("overflow-hidden");
    }
}
...
function setTrends(data, category, container){
    var _mId = container.split("_")[1];
    var _cId = container.split("_")[2];
    var _trendId = "trend_" + _mId +"_"+ _cId;
    var _ndpId = "ndp_" + _mId +"_"+ _cId;
    var _evoId = "opp_" + _mId +"_"+ _cId;
    ...
    if(data.length > 0){
        ...
        _display( $("div#"+ _trendId),_aTrends );
        _display( $("div#"+ _ndpId),_aBestpractice );   
        _display( $("div#"+ _evoId),_aEvolution );   
    }
}
...
function displayComPieChart(container){
    ...
    setLegend(_charts, container);
    setTrends(_o.data, gModelYears, container);
}
...
function displayComPieRegionChart(container){
    ...
    var _container = "#"+ container;
    var _chartWidth = $("#"+ container).parent().width() / 3;
    var _tw = new zsi.easyJsTemplateWriter(_container);
    
    $.each(_o.selectedCategory, function(i, v){
        var _cName = v.name;
        var _cNameNew = _cName.replace(/ /g,"_");
        var _chartDiv = container +"_"+ _cNameNew;
        ...
        $.each(gModelYears, function(i, my){
            var _name = my.name;
            var _chartSubDiv = _chartDiv +"_"+ _name;
    ...
    setLegend(_charts, container);
    setTrends(_o.data, _o.selectedCategory, container);
}
...
function displayComColumnChart(container){
    ...
    setLegend(chart, container);
    setTrends(o.data, o.categoryObj, container);
}
...
function displayComMapChart(container){
    ...
    setTrends(_data, _categoryKeyObj, container);
}
...
function getColumnChartDataDD(o){
    ...
    var _$cFlex = $("#" + o.container).closest(".flex-chart");
    var _$chartWrapper = _$cFlex.find(".chart-wrapper");
    var _$container = _$cFlex.find(".bar_details");
    var _containerID = _$container.attr("id");
    var _$selectedCategory = _$cFlex.find(".selected_category");
    var _$optionBox = _$cFlex.find(".chart_details_opt");
...
function displayDDColumn(o){
    ...
    $.each(_o.locationObj, function(i, v) { 
        var _name = v.name;
        var _nameNew = _name.replace(/ /g,"_");
...
function displayNCTLesserDimensions(container, callback){
    ...
    var _container = "#"+ container;
    var _chartWidth = $(_container).parent().width();
    var _tw = new zsi.easyJsTemplateWriter(_container);
    ...
    setCustomTrend(_trendData, container);
}
...
function setCustomTrend(data, container){
    var _mId = container.split("_")[1];
    var _cId = container.split("_")[2];
    var _trendId = "trend_" + _mId +"_"+ _cId;
    var _ndpId = "ndp_" + _mId +"_"+ _cId;

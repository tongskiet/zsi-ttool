//(function(){
    ...
    zsi.ready = function(){
        ...
        var _res = gTrendMenuItems.filter(function(item){ return $.trim(item.name)==="M" })[0].items;
        displayMenus(_res);
        setInfoEvents();
    };
        ...
        if(gPrmCategory==="Region" || gPrmCategory==="Market"){
            ...
            _html += "<tfoot>"
                    ...
                    $.each(_selectedCategory, function(i, z){
                        ...
                        var _overAllTotal = getCount(_res4, "value");
                        _html += "<th class='text-right'>"+ (gMenu==="Coverings" ? convertDecimalToString(_overAllTotal) : _overAllTotal) +"</th>";
                    });
            _html += "</tr></tfoot>";
        }
        _html += "</table>";
    ...
    function setCriteriaParams(callback){
        ...
        if(gCriteriaParams.length > 0){
            var _$paramBox = $(".section-card[cid='"+ gCId +"']").find(".param_box");
            ...
            $.each(rows, function(x, o){
                ...
                $.each(_menuItems, function(i, v){
                    ...
                    if(x===0 && i===0){
                        ...
                        $("#menuMechanical").find(".section:first-child()").find(".section-card:first-child() .card-header").addClass("active");
                    }
                });
            })
        }

CREATE procedure [dbo].[wires_count_summary] (
...
SET NOCOUNT ON
   ...
   set @stmt = 'SELECT COUNT(WIRE_GAUGE) small_wire_count, ' + @cols + ' FROM dbo.cutsheets_v WHERE CONVERT(decimal(5,2),WIRE_GAUGE) <= .5 '
...
--[small_wires_count_summary] @byMY='Y', @byRegion='Y',@model_year_fr = 2017
--[small_wires_count_summary] @byRegion='Y',@model_year_fr = 2017
